...
vec4 ssaoVL_blur(vec2 tex,vec2 dir,float cdepth)
{
	...
	vec4 sp=texture2D(colortex0,tex-2.*step);
	float linD=abs(cdepth-ld(texture2D(depthtex0,tex-2.*step).x)*far);
	float ssaoThresh=linD<7.5?1.:0.;
...
void main(){
	...
	color=color*(1.-transparencies.a)+transparencies.rgb;
	
	vec4 vl=ssaoVL_blur(texcoord,vec2(1.,0.),ld(Depth)*far);
	color*=vl.a/50000.;
	color+=vl.rgb;
