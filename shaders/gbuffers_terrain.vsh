...
void main(){
	...
	vtexcoord.xy=sign(texcoordminusmid)*.5+.5;
	#endif
	...
	normalMat=vec4(normalize(gl_NormalMatrix*gl_Normal),mc_Entity.x==10004||mc_Entity.x==10003||mc_Entity.x==10001?0.:1.);
	#ifdef WAVY_PLANTS
	...
	if((mc_Entity.x==10003)){
		...
		position=gbufferModelViewInverse*position;
		vec3 worldpos=position.xyz+cameraPosition;
		position.xyz+=calcMoveLeaves(worldpos.xyz,.0040,.0064,.0043,.0035,.0037,.0041,vec3(1.,.2,1.),vec3(.5,.1,.5))*lmtexcoord.w;
		position=gbufferModelView*position;
		gl_Position=gl_ProjectionMatrix*position;
	}
	#endif
