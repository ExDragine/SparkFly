...
#ifdef PCF
const vec2 shadowOffsets[4]=vec2[4](vec2(.1250,0.),
vec2(-.1768,-.1768),
vec2(-0.,.3750),
vec2(.3536,-.3536)
);
...
if(gl_FragData[0].a>.1){
	vec3 albedo=toLinear(gl_FragData[0].rgb*color.rgb);
	
	vec3 normal=normalMat.xyz;
	vec3 fragpos=toScreenSpace(gl_FragCoord.xyz*vec3(texelSize,1.)-vec3(vec2(tempOffset)*texelSize*.5,0.));
	
	float NdotL=lightCol.a*dot(normal,sunVec);
	float NdotU=dot(upVec,normal);
	float diffuseSun=clamp(NdotL,0.f,1.f);
	float skyLight=(NdotU*.4+1.)*(1.-rainStrength*.8);
	
	diffuseSun=diffuseSun+(.5*-NdotL+.5+abs(NdotL)*pow(clamp(lightCol.a*dot(normalize(fragpos),sunVec),0.f,1.f),60.)*10.);
	...
	if(diffuseSun>.001){
		...
		if(abs(projectedShadowPosition.x)<1.-1.5/shadowMapResolution&&abs(projectedShadowPosition.y)<1.-1.5/shadowMapResolution){
			const float sr=1024./shadowMapResolution;
			float diffthresh=.000125;
			
			projectedShadowPosition=projectedShadowPosition*vec3(.5,.5,.5/3.)+vec3(.5,.5,.5);
			
			#ifdef PCF
			
			float noise=interleaved_gradientNoise(tempOffset.x*.5+.5);
			
			vec2 offsetS=vec2(cos(noise*3.14159265359*2.),sin(noise*3.14159265359*2.));
			
			float shading=shadow2D_bicubic(shadow,vec3(projectedShadowPosition+vec3(0.,0.,-diffthresh*1.2)));
			#endif
	...
	vec3 ambient=(lightCol.a*sunElevation)*(-NdotL*.45+.9)*lightCol.rgb*.6+(skyLight*skyIntensity)*vec3(.65,.7,1.)*30.+skyIntensityNight*vec3(.09,.1,.15)/1.5;
	
	vec3 diffuseLight=direct+(lmtexcoord.w)*ambient+vec3(TORCH_R,TORCH_G,TORCH_B)*torch_lightmap*.8*TORCH_AMOUNT+.0006*MIN_LIGHT_AMOUNT;
