...
float interleaved_gradientNoise(float temporal){
	...
	float noise=fract(52.9829189*fract(.06711056*coord.x+.00583715*coord.y)+temporal);
	return noise;
}
...
vec3 rayTrace(vec3 dir,vec3 position,float dither){
	...
	float maxZ=spos.z+stepv.z*.5;
	spos.xy+=offsets[framemod8]*texelSize*.5;
	...
	for(int i=0;i<int(quality+1);i++){
		...
		if(sp<=max(maxZ,minZ)&&sp>=min(maxZ,minZ)){
			...
			#endif
			#ifndef FULL_RES_DEPTH_BUFFER
			return vec3(spos.xy*4.,sp);
			#endif
		}
		spos+=stepv;
		//small bias
		minZ=maxZ-.00004/ld(spos.z);
		maxZ+=stepv.z;
	}
...
float GetNoHSquared(float radiusTan,float NoL,float NoV,float VoL)
{
	...
	float radiusCos=inversesqrt(1.+radiusTan*radiusTan);
	...
	float rOverLengthT=radiusCos*radiusTan*inversesqrt(1.-RoL*RoL);
	...
	float triple=sqrt(clamp(1.-NoL*NoL-NoV*NoV-VoL*VoL+2.*NoL*NoV*VoL,0.,1.));
	...
	float xNum=q*(-.5*p+.25*VoBr*NoLVTr);
	float xDenom=p*p+s*((s-2.*p))+NoLVTr*((NoL*radiusCos+NoV)*VoLVTr*VoLVTr+
	q*(-.5*(VoLVTr+VoL*radiusCos)-.5));
...
float GGX(vec3 n,vec3 v,vec3 l,float r,float F0,float lightSize){
	...
	return dotNL*D*F/(dotLH*dotLH*(1.-k2)+k2);
}
...
float cdist(vec2 coord){
	return max(abs(coord.s-.5),abs(coord.t-.5))*2.;
}
float w0(float a)
{
...
float shadow2D_bicubic(sampler2DShadow tex,vec3 sc)
{
	...
	vec2 p0=vec2(iuv.x+h0x,iuv.y+h0y)/shadowMapResolution-.5/shadowMapResolution;
	vec2 p1=vec2(iuv.x+h1x,iuv.y+h0y)/shadowMapResolution-.5/shadowMapResolution;
	vec2 p2=vec2(iuv.x+h0x,iuv.y+h1y)/shadowMapResolution-.5/shadowMapResolution;
	vec2 p3=vec2(iuv.x+h1x,iuv.y+h1y)/shadowMapResolution-.5/shadowMapResolution;
...
vec3 getParallaxDisplacement(vec3 posxz,float iswater,float bumpmult,vec3 viewVec){
	...
	vec2 vec=viewVector.xy*(1./float(PW_POINTS))*22.*PW_DEPTH;
	float waterHeight=getWaterHeightmap(posxz.xz,waveM,waveZ,iswater)*.5;
...
void main() {
	...
	if (iswater > 0.0){
		...
		*/
		float sunSpec=GGX(normal,-normalize(fragpos),lightCol.a*sunVec,roughness,f0,lightCol.a>0.?.035:.065)*.01;
		
		vec3 wrefl=mat3(gbufferModelViewInverse)*reflectedVector;
		vec3 sky_c=getSkyColor(wrefl,mat3(gbufferModelViewInverse)*sunVec,wrefl.y)*lmtexcoord.w*lmtexcoord.w*1.4*(1.-isEyeInWater)*(2.5-exp(-fogAmount*256.)*2.+fogAmount*100.*.3);
		...
		if(rtPos.z<1.05){
			...
			previousPosition.xy=previousPosition.xy/previousPosition.w*.5+.5;
			reflection.a=clamp(1.-pow(cdist(previousPosition.st),20.),0.,1.);
		...
		vec3 specColor=mix(normalize(albedo+.00001)*.25,vec3(1./sqrt(3.)),1.-alpha0*alpha0*alpha0*alpha0);
		
		//correct alpha channel with fresnel
		gl_FragData[0].a=-gl_FragData[0].a*fresnel+gl_FragData[0].a+fresnel;
		gl_FragData[0].rgb=clamp(color*10./gl_FragData[0].a*alpha0*(1.-fresnel)+reflected*10./gl_FragData[0].a,0.,65100.);
