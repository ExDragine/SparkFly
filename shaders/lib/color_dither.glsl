...
float triangWhiteNoise(vec2 n)
{
	...
	float rnd=nrand(n+.07*t);
	
	float center=rnd*2.-1.;
...
vec3 int8Dither(vec3 color,vec2 tc01){
	float dither=triangWhiteNoise(tc01);
	return color+dither*exp2(-8.);
}

vec3 int10Dither(vec3 color,vec2 tc01){
	float dither=triangWhiteNoise(tc01);
	return color+dither*exp2(-10.);
}

vec3 int16Dither(vec3 color,vec2 tc01){
	float dither=triangWhiteNoise(tc01);
	return color+dither*exp2(-16.);
}

vec3 int32Dither(vec3 color,vec2 tc01){
	float dither=triangWhiteNoise(tc01);
	return color+dither*exp2(-32.);
}