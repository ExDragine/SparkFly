...
if(gl_FragData[0].a>.1){
	...
	if(diffuseSun>.001){
		...
		if(abs(projectedShadowPosition.x)<1.-1.5/shadowMapResolution&&abs(projectedShadowPosition.y)<1.-1.5/shadowMapResolution){
			...
			float diffthresh=(facos(diffuseSun)*.006*sr+.00006*sr)/(distortFactor*distortFactor);
	...
	vec3 ambient=(lightCol.a*sunElevation)*(-NdotL*.45+.9)*lightCol.rgb*1.25+(skyLight*skyIntensity)*vec3(.65,.7,1.)*20.+skyIntensityNight*vec3(.09,.1,.15)/1.5;
	...
	vec3 color=diffuseLight*albedo*.7;
