...
if(gl_FragData[0].a>.1){
	...
	vec3 fragpos=toScreenSpace(gl_FragCoord.xyz*vec3(texelSize,1.)-vec3(vec2(tempOffset)*texelSize*.5,-.38));
	...
	vec3 ambient=(lightCol.a*sunElevation)*(-NdotL*.45+.9)*lightCol.rgb*.5+(skyLight*skyIntensity)*vec3(.5,.6,1.)*20.+skyIntensityNight*vec3(.09,.1,.15)/1.5;
