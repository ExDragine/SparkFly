...
void main() {
	...
	if (gl_FragData[0].a>0.1){
		...
		vec3 fragpos = toScreenSpace(gl_FragCoord.xyz*vec3(texelSize,1.0)-vec3(vec2(tempOffset)*texelSize*0.5,-0.38));
		...
		vec3 ambient = (lightCol.a*sunElevation)*(-NdotL*0.45+0.9)*lightCol.rgb*0.5 + (skyLight*skyIntensity)*vec3(0.5,0.6,1.)*20. + skyIntensityNight*vec3(0.09,0.1,0.15)/1.5;
